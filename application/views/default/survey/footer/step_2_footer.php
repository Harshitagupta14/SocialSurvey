...
//    mq_choice_low_label.innerHTML = low_value;
    ...
    if (array_length < qLimitUp && array_length <= 2) {
    var error = "Number of muliple options must not be greater than Max choice and less than two";
    display_error(error);
    return false;
    }
    }
    ...
    if (type == "mq") {
    $("#mq-block").show();
    var mq_choice_html = create_mq( "Max Choice");
    $("#question_block").append(mq_choice_html);
    }
    if (type == "nm") {
